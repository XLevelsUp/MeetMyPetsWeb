...
import { updateBreed } from "@/lib/taxonomy";
import { updateBreedBodySchema } from "@/lib/taxonomy-contract";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = updateBreedBodySchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json<ApiError>(
  ...
  const result = await updateBreed(id, input, reason, {
