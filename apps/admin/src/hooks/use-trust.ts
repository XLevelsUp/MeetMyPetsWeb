...
/**
 * Restore or permanently ban. Invalidates the queue, the ledger, and
 * `["users"]` — a pet's trust and its restriction badge are shown on the report
 * queue and the pets table too, so refreshing only this screen would leave the
 * rest of the panel showing state that has just changed.
 */
export function useTrustAction() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (input: { petId: string; action: "restore" | "ban"; reason: string }) => {
